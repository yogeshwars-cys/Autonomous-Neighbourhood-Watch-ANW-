...
//
// EpisodeDigest is the answer: a small, selective, TTL-bounded unit of
...
// EpisodeDigest is the network-safe, compact representation of an Episode.
// Deliberately smaller than Episode itself (no free-text Note) for the
...
func SelectForGossip(selfID string, mem *EpisodicMemory, now time.Time) []EpisodeDigest {
	if mem == nil {
		return nil
	}
	...
	out := make([]EpisodeDigest, len(top))
	for i, e := range top {
		out[i] = EpisodeDigest{
			OriginID:    selfID,
			Kind:        e.Kind,
...
func RelayCandidates(selfID string, received []EpisodeDigest) []EpisodeDigest {
	out := make([]EpisodeDigest, 0, len(received))
...
func appendBounded(s []EpisodeDigest, v EpisodeDigest, limit int) []EpisodeDigest {
	s = append(s, v)
	if len(s) > limit {
		s = s[len(s)-limit:]
	}
