...
type Heartbeat struct {
	...
	// Events piggybacks a small, selective batch of episode digests —
	// Objective 7/9's "smarter gossip" (see episode_gossip.go). Exactly
	// the same tradeoff KnownPeers already made for addresses in
	// Milestone 3, applied to remembered episodes instead: a slightly
	// larger packet, in exchange for situational awareness propagating
	// without any central collector. Omitted entirely on ticks with
	// nothing worth sharing, so quiet agents cost the network nothing
	// extra.
	Events []EpisodeDigest `json:"events,omitempty"`

	// ActiveEvent (Milestone 6) is this agent's CURRENT semantic event
	// summary, if any — a fundamentally different kind of information
	// from Events above: Events are a selective sample of PAST episodes
	// (Objective 9's memory), ActiveEvent is "what do you think is
	// happening RIGHT NOW." Nil whenever event detection is disabled or
	// nothing is currently abnormal — never fabricated to keep the
	// field non-nil. Only ever an *EventSummary (event.go), the same
	// narrow, conclusion-only shape every other field on this struct
	// already uses: a peer learns the agent's classification and
	// confidence, never the FeatureVector or raw Observations that
	// produced it.
	ActiveEvent *EventSummary `json:"active_event,omitempty"`
