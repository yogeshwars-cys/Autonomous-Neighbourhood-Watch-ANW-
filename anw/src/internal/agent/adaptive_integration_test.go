...
// and leaves ALERT should end up with EpisodeSpike and EpisodeRecovery
...
func TestEpisodicMemoryRecordsRealAlertEpisode(t *testing.T) {
	...
	for _, e := range a.Memory.Recent(a.Memory.Count()) {
		if e.Kind == EpisodeSpike {
			sawSpike = true
		}
	}
	if !sawSpike {
		t.Errorf("expected memory to contain at least one EpisodeSpike, got %v", a.Memory.Recent(a.Memory.Count()))
	}
}
