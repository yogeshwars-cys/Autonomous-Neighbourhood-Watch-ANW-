...
func evtAt(kind EpisodeKind, danger float64, secOffset int) Episode {
	return Episode{
...
func TestEpisodicMemoryRecordAndCount(t *testing.T) {
	...
	m.Record(evtAt(EpisodeSpike, 0.8, 0))
	m.Record(evtAt(EpisodeRecovery, 0.1, 1))
	if m.Count() != 2 {
...
func TestEpisodicMemoryCapacityIsBounded(t *testing.T) {
	...
	for i := 0; i < episodicCapacity+50; i++ {
		...
		m.Record(evtAt(EpisodeSpike, 0.9, i))
	}
...
func TestEpisodicMemoryForgetsLowImportanceOverTime(t *testing.T) {
	...
	m.Record(Episode{Timestamp: base, Kind: EpisodeRecovery, DangerScore: 0.1})
...
func TestEpisodicMemoryRemembersSevereEventsLonger(t *testing.T) {
	m := NewEpisodicMemory()
	base := time.Unix(0, 0)
	m.Record(Episode{Timestamp: base, Kind: EpisodeSustained, DangerScore: 1.0})
...
func TestEpisodicMemoryRecentOrderingIsOldestFirst(t *testing.T) {
	m := NewEpisodicMemory()
	m.Record(evtAt(EpisodeSpike, 0.5, 0))
	m.Record(evtAt(EpisodeRecovery, 0.1, 1))
	m.Record(evtAt(EpisodeSpike, 0.6, 2))

	recent := m.Recent(2)
	if len(recent) != 2 {
		t.Fatalf("expected 2 recent events, got %d", len(recent))
	}
	if recent[0].Kind != EpisodeRecovery || recent[1].Kind != EpisodeSpike {
...
func TestEpisodicMemoryTopImportantRanksBySeverityAndRecency(t *testing.T) {
	...
	// An old, mild event...
	m.Record(Episode{Timestamp: base.Add(-30 * time.Second), Kind: EpisodeRecovery, DangerScore: 0.1})
	...
	if top[0].Kind != EpisodeSustained {
...
func TestEpisodicMemorySummaryMentionsMostRecentEvent(t *testing.T) {
	...
	m.Record(evtAt(EpisodeSpike, 0.7, 0))
