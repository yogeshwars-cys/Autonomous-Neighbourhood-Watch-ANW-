...
func TestSelectForGossipCapsAtMaxGossipEvents(t *testing.T) {
	...
	for i := 0; i < maxGossipEvents+10; i++ {
		m.Record(Episode{Timestamp: now, Kind: EpisodeSpike, DangerScore: 0.9})
	}
...
func TestSelectForGossipStampsOriginAndZeroHops(t *testing.T) {
	m := NewEpisodicMemory()
	now := time.Unix(1000, 0)
	m.Record(Episode{Timestamp: now, Kind: EpisodeSpike, DangerScore: 0.5})
...
func TestRelayCandidatesIncrementsHops(t *testing.T) {
	digests := []EpisodeDigest{{OriginID: "node-b", Hops: 1}}
	got := RelayCandidates("node-a", digests)
	if len(got) != 1 {
...
func TestAppendBoundedTrimsFromFront(t *testing.T) {
	var s []EpisodeDigest
	for i := 0; i < 5; i++ {
		s = appendBounded(s, EpisodeDigest{OriginID: "node-a", Hops: i}, 3)
	}
	if len(s) != 3 {
